...
export async function openOrder(web3: Web3, walletDetails: WalletDetail, address: string, order: Order): Promise<Order> {
    ...
    try {
        signature = new EncodedData(await walletDetails.sign(web3, hashForSigning, address));
    } catch (error) {
...
export async function cancelOrder(web3: Web3, walletDetails: WalletDetail, address: string, orderId64: string): Promise<{}> {
    ...
    let signature: EncodedData;
    try {
        signature = new EncodedData(await walletDetails.sign(web3, hashForSigning, address), Encodings.HEX);
    } catch (error) {
        if (error.message.match(/User denied message signature/)) {
            return Promise.reject(new Error(ErrorCanceledByUser));
        }
...
export async function buildOrderFragmentsForPods(
    web3: Web3, darknodeRegistryContract: DarknodeRegistryContractType, order: Order
...
export function encryptForDarknode(darknodeKey: NodeRSA | null, share: shamir.Share, byteCount: number): EncodedData {
    if (darknodeKey === null) {
 ...
 */
async function getPods(web3: Web3, darknodeRegistryContract: DarknodeRegistryContractType): Promise<List<Pool>> {
    const darknodes = await darknodeRegistryContract.methods.getDarknodes().call();
    const minimumPodSize = parseInt(await darknodeRegistryContract.methods.minimumPodSize().call(), 10);
    const epoch = await darknodeRegistryContract.methods.currentEpoch().call();
    ...
    for (let i = 0; i < darknodes.length; i++) {
        let isRegistered = await darknodeRegistryContract.methods.isRegistered(darknodes[x.toNumber()]).call();
        while (!isRegistered || positionInOcean.get(x.toNumber()) !== -1) {
            x = x.add(new BN(1));
            x = x.mod(numberOfDarknodes);
            isRegistered = await darknodeRegistryContract.methods.isRegistered(darknodes[x.toNumber()]).call();
        }
